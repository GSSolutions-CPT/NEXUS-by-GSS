...
export default function Login() {
    ...
    const [rememberMe, setRememberMe] = useState(false); // #4 — Remember Me
    const [loading, setLoading] = useState(false);
    const [redirecting, setRedirecting] = useState(false); // #1 — session check state
    ...
    // #1 — On mount, check if user already has a valid session and redirect
    useEffect(() => {
        const checkSession = async () => {
            ...
            else if (profile.role === "GroupAdmin") router.replace("/owner");
    ...
    const redirectByRole = (role: string) => {
        ...
        else if (role === "GroupAdmin") router.push("/owner");
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        // #3 — Block if locked out
        if (lockoutRemaining > 0) {
            setError(`Too many failed attempts. Please wait ${lockoutRemaining}s.`);
            return;
        }
        ...
        try {
            ...
            if (authError || !data.user) {
                ...
                if (newAttempts >= MAX_ATTEMPTS) {
                    startLockout();
                    throw new Error(`Too many failed attempts. Please wait ${LOCKOUT_SECONDS} seconds before trying again.`);
                }
            ...
            // #2 — Retry profile fetch up to 3 times (handles race condition where
            // the profile row hasn't been created yet by the DB trigger)
            let profile = null;
            for (let i = 0; i < 3; i++) {
            ...
            if (!profile) throw new Error("Your account is still being set up. Please wait a moment and try again, or contact support.");
            ...
            <main className="relative z-10 flex w-full max-w-[440px] flex-col items-center justify-center px-4 py-8">
                                    ...
                                    <form onSubmit={handleLogin} className="w-full space-y-4 text-left">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div className="relative group/input">
                                                <input type="email" placeholder="admin@globalsecurity.co.za" value={email}
                                                    onChange={e => setEmail(e.target.value)} disabled={loading || lockoutRemaining > 0} required
                                        ...
                                        <div className="flex items-center gap-2.5 ml-1">
                                            <input
                                                ...
                                                className="w-4 h-4 rounded bg-white/[0.03] border border-white/[0.08] text-sky-500 focus:ring-sky-500/30 cursor-pointer"
                                            />
                                            <label htmlFor="remember-me" className="text-[12px] text-slate-400 cursor-pointer select-none">
                                                Keep me signed in
                                            </label>
                                        </div>
                                        ...
                                        {/* #3 — Lockout warning */}
                                        {lockoutRemaining > 0 && (
                                            <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl text-amber-400 text-xs font-medium text-center">
                            ...
                            {mode === "forgot-sent" && (
                                <div className="w-full text-center space-y-6 py-4">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        {/* #7 — note about expiry */}
                                        <p className="text-xs text-amber-400/70 font-medium pt-1">⚠ The reset link expires in 24 hours.</p>
                                    </div>
                ...
                <div className="mt-8 text-center">
                    ...
                    <div className="mt-5 flex items-center justify-center gap-4 text-xs font-medium">
                        <a href="https://wa.me/27629558559" target="_blank" rel="noopener noreferrer" className="text-slate-500/70 hover:text-slate-300 transition-colors flex items-center gap-1.5">
