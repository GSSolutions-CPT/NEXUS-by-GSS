...
import { portalLogin, portalSaveMaster } from '@/lib/impro/client';
...
export async function POST(request: Request) {
    try {
        ...
        // DIRECT IMPRO PORTAL SYNC: Bypass the C# Bridge and sync directly to the Portal XML API
        try {
            ...
            // We use the pin as the ID Number so we can reference it
            const idNumber = generatedPin;
            
            await portalLogin({ host: portalHost, username: portalUser, password: portalPass });
            await portalSaveMaster({
                // State 0 = new
                state: 0,
                ...
                lastName,
                idNumber,
                tagCode: generatedPin // Pin code used as tag
            }, ['tags']);
            console.log(`[${user.id}] Successfully synced visitor ${firstName} to Impro Portal directly.`);
        } catch (syncErr) {
            console.error(`[${user.id}] Direct Portal Sync failed:`, syncErr);
            // We don't fail the request, the local DB insert still succeeded
        }
